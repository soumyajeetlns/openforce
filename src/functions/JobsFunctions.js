...
export function getJobsByStatusWithPending(employerID, status, successCallback,errorCallback){
    FirebaseRef.collection(tables.TABLE_JOBS).where("employerID", "==", employerID).where("status", "==", status).orderBy("postedDate", "desc").onSnapshot((querySnapshot) => {
        const jobs = [];
...
export function getPaymentRequestData(jobId, paymentId, successCallback,errorCallback){
    const ref = FirebaseRef.collection(tables.TABLE_JOBS).doc(jobId).collection(tables.SUB_TABLE_PAYMENT).doc(paymentId);
...
export function getEmployeeData(empId, successCallback,errorCallback){
    const ref = FirebaseRef.collection(tables.TABLE_USERS).doc(empId);
...
export function updatePaymentStatus(jobId, empId, paymentStatus, successCallback,errorCallback){
    FirebaseRef.collection(tables.TABLE_JOBS).doc(jobId).collection(tables.SUB_TABLE_PAYMENT).doc(empId).update({status: paymentStatus, actionDate: Date.now()}).then((docRef) =>{
        if(successCallback)successCallback(docRef);
    }).catch((error)=> {
        if(errorCallback)errorCallback(error);
        utils.logError(error)
